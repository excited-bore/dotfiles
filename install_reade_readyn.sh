...
if ! test -f $file; then
    ...
    #reade -Q 'GREEN' -i 'y' -p 'Install rm-prompt? (Rm but prompts files before deletion) [Y/n]: ' 'n' rm_ins
        ...
        echo "Next $(tput setaf 1)sudo$(tput sgr0) will install $(tput setaf 2)reade$(tput sgr0) and$(tput setaf 2) readyn$(tput sgr0) inside $(tput bold)/usr/bin/$(tput sgr0)"
        sudo install -Dm777 $file -t "/usr/bin/" 
        sudo install -Dm777 $file1 -t "/usr/bin/" 
    #fi
#fi
