...
function git_hl() {
    ...
    if [[ $cmd == 'lazygit' ]] && hash difft &>/dev/null; then
        diffs=$diffs" difftastic"
    fi
    if hash ydiff &>/dev/null; then
        diffs=$diffs" ydiff"
    fi
    if hash riff &>/dev/null; then
        diffs=$diffs" riff"
    fi
    if hash diffr &>/dev/null; then
        diffs=$diffs" diffr"
    fi
    ...
    if test -n "$diff"; then
        local opts='' colorArg='always' side='n' extrn='n'
        if [[ "$diff" == "delta" ]]; then
            opts=' --paging=never'
...
git_pager() {
    ...
    if [[ $pager == 'cat' ]]; then
        ...
        readyn -Y 'CYAN' -p "You chose $pager. Configure?" moor_cnf
        if [[ $moor_cnf == 'y' ]]; then
            ...
            while :; do
                ...
                readyn -Y "CYAN" -p "Set moor's theme / style?" pager1
                if [[ $pager1 == 'y' ]]; then

                    # Showing style changes using fzf's --preview doesn't work with moor,
                    # so it's this mess
                    local style thme='n' styles="abap\nalgol\nalgol_nu\napi\narduino\nautumn\naverage\nbase16-snazzy\nborland\nbw\ncatppuccin-frappe\ncatppuccin-latte\ncatppuccin-macchiato\ncatppuccin-mocha\ncolorful\ncompat\ndoom-one\ndoom-one2\ndracula\nemacs\nfriendly\nfruity\ngithub-dark\ngithub\ngruvbox-light\ngruvbox\nhr_high_contrast\nhrdark\nigor\nlovelace\nmanni\nmodus-operandi\nmodus-vivendi\nmonokai\nmonokailight\nmurphy\nnative\nnord\nonedark\nonesenterprise\nparaiso-dark\nparaiso-light\npastie\nperldoc\npygments\nrainbow_dash\nrose-pine-dawn\nrose-pine-moon\nrose-pine\nrrt\nsolarized-dark\nsolarized-dark256\nsolarized-light\nswapoff\ntango\ntrac\nvim\nvs\nvulcan\nwitchhazel\nxcode-dark\nxcode"
                    while [[ $thme == 'n' ]]; do
                        style=$(printf "$styles" | fzf --reverse --border --border-label="moor style" --preview="moor --style={} $TMPDIR/test1")
                        moor='' moor --style="$style" $TMPDIR/test1
                ...
                readyn -Y "MAGENTA" -p "Is this ok? (Will retry if no)" moor_ok
...
${CYAN}\t4)${normal}${green} Disable pager/just highlight\n${normal}"
            ...
            if ! [[ "$choice" == '4' ]]; then
                ...
                if test -z "$pgr" || ! hash $pgr &>/dev/null; then
                    if test -n "$PAGER" && hash $(echo $PAGER | awk '{print $1;}') &>/dev/null; then
                ...
                if [[ "$choice" == '1' ]]; then
                    ...
                    if [[ $pgr =~ "less" ]]; then
                        ...
                        pgr='moor'
                        printf "${CYAN}You selected ${GREEN}moor${normal}\n"
