...
if test -z "$yt_dl" && hash yt-dlp &> /dev/null || hash youtube-dl &> /dev/null; then
    ...
    function yt-dl(){
        ...
        local playlist='--no-playlist' start=" " end=" " plst_frm='' rror='' list_sub sub_list cap_list cap_frm cap_frm_p live_chat="" sub_langs sub_lang sub='' cap='' format="" format_sub="" format_cap="" format_all="" form_pre sub_cap format_sub='' frst_frm frst_frm_p frst_pre lang sub_auto=" " cap_langs caplang plslt plst_frm plst_dir plst_rank 
        ...
        if [[ "$sub_cap" == 'y' || "$sub_cap" == 'sub' || "$sub_cap" == 'cap' ]]; then
        ...
        if [[ $min_res == 'y' ]]; then
            ...
            reade -Q 'GREEN' -i '1080 720 480 360 240 144' -p 'Minimum resolution: ' res
