...
if grep -q "#EnableAUR" /etc/pamac.conf; then
    reade -Q "GREEN" -i "y" -p "Enable AUR for pamac? [Y/n]: " "y n" aurset
    if [ "$aurset" == "y" ]; then
...
if grep -q "#CheckAURUpdates" /etc/pamac.conf; then
    reade -Q "GREEN" -i "y" -p "Enable updatecheck for AUR packages? [Y/n]: " "y n" aurset1
    if [ "$aurset1" == "y" ]; then
...
if grep -q "#CheckAURVCSUpdates" /etc/pamac.conf; then
    reade -Q "GREEN" -i "y" -p "Enable updatecheck for AUR Development packages? [Y/n]: " "y n" aurset1
    if [ "$aurset1" == "y" ]; then
...
if test -x "$(command -v flatpak)" && grep -q "#EnableFlatpak" /etc/pamac.conf; then
    ...
    if [ "$fltpak" == "y" ]; then
        ...
        if grep -q "#CheckFlatpakUpdates" /etc/pamac.conf; then
            reade -Q "GREEN" -i "y" -p "Enable updatecheck for flatpak updates? [Y/n]: " "y n" fltpak1
            if [ "$fltpak1" == "y" ]; then
