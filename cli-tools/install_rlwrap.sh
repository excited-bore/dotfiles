    ...
    if [[ "$answr" == "y" ]]; then
        if [[ $machine == 'Windows' ]] && hash pacman &>/dev/null; then
            ...
            if ! hash apt-cyg &>/dev/null; then
                if ! [[ -f $TOP/cli-tools/pkgmngrs/install_apt-cyg.sh ]]; then
                    source <(wget-curl https://raw.githubusercontent.com/excited-bore/dotfiles/main/cli-tools/pkgmngrs/install_apt-cyg.sh)
                else
                    . $TOP/cli-tools/pkgmngrs/install_apt-cyg.sh
                fi
            fi
