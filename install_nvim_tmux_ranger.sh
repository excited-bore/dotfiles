...
do
    if [ -f $f ] && [ $f == /etc/arch-release ];then
        ...
        echo Package manager: ${osInfo[$f]}
        pm=${osInfo[$f]}
        sudo apt install flatpak build-essential python2 python3 cmake neovim python3-dev python3-pip mono-complete golang nodejs openjdk-17-jdk openjdk-17-jre npm ranger atool bat elinks ffmpegthumbnailer fontforge highlight imagemagick jq libcaca0 odt2txt mupdf-tools 
        fi 
    done
    ...
    doas -u $USER nvim +PluginInstall +qall
    ...
    echo "Don't forget to open tmux and Prefix+I !"
