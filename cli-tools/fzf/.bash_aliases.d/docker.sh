...
fzf-docker-new-container(){ docker-fzf-new-container; }

docker-fzf-open-container() {
  export FZF_DEFAULT_OPTS='--height 90% --reverse --border'
...
fzf-docker-open-container(){ docker-fzf-open-container; }

docker-fzf-stop-container() {
  export FZF_DEFAULT_OPTS='--height 90% --reverse --border'
  local container=$(docker ps --format '{{.Names}} => {{.Image}}' | fzf-tmux --reverse --multi | awk -F '\\=>' '{print $1}')
  if [[ $container != '' ]]; then
    echo -e "\n  \033[1mDocker container:\033[0m" $container
...
fzf-docker-stop-container(){ docker-fzf-stop-container; }

docker-fzf-show-ip-container() {
  export FZF_DEFAULT_OPTS='--height 90% --reverse --border'
...
fzf-docker-show-ip-container(){ docker-fzf-show-ip-container; }
