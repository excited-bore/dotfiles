...
git-https-to-ssh() {
    if [ -z $1 ]; then
        ...
        if [[ $resp == 'y' ]]; then
            gitRm=$(git remote get-url origin | sed 's,.*.com/,git@github.com:,g')
            git remote -v set-url origin $gitRm
            git remote get-url origin
        fi
    else
        gitRm=$(git remote get-url $1 | sed 's,.*.com/,git@github.com:,g')
        git remote -v set-url $1 $gitRm
...
git-ssh-to-https() {
    if [ -z $1 ]; then
        ...
        readyn -p "Do you want me to look for 'origin'?" resp
        if [[ $resp == 'y' ]]; then
            gitRm=$(git remote get-url origin | sed 's,.*.com:,https://github.com/,g')
            git remote -v set-url origin $gitRm
            git remote get-url origin
        fi
    else
        gitRm=$(git remote get-url $1 | sed 's,.*.com:,https://github.com/,g')
        git remote -v set-url $1 $gitRm
        git remote get-url $1
    fi
}
