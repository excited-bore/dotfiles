...
do
    if [ -f $f ] && [ $f == /etc/arch-release ];then
            ...
            pm=${osInfo[$f]}
            sudo pacman -Su xclip sshfs neovim mono go nodejs jre11-openjdk npm python ranger atool bat calibre elinks ffmpegthumbnailer fontforge highlight imagemagick kitty mupdf-tools odt2txt 
    ...
    sudo -u $USER nvim +PluginInstall +qall
