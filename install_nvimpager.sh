...
if [ -z "$moar_usr" ] || [ "y" == "$moar_usr" ] || [ "Y" == "$moar_usr" ]; then
    if grep -q "nvimpager" $PATHVAR; then 
        ...
        sed -i "s|export PAGER=.*|export PAGER=$(whereis nvimpager)|g" $PATHVAR
        ...
        printf "export PAGER=$(whereis nvimpager)\n" >> $PATHVAR
...
if [ -z "$moar_root" ] || [ "y" == "$moar_root" ] || [ "Y" == "$moar_root" ]; then
    if sudo grep -q "nvimpager" $PATHVAR_R; then
        ...
        sudo sed -i "s|export PAGER=.*|export PAGER=$(whereis nvimpager)|g" $PATHVAR_R
