...
echo "Next $(tput setaf 1)sudo$(tput sgr0) will install '.aliases.d' in /root and source it with '/root/.bash_aliases' in /root/.bashrc"
...
if sudo [ -f /root/.bashrc ] && ! sudo grep -q '\[ -f ~/.bash_aliases \] && source ~/.bash_aliases' /root/.bashrc; then
...
if sudo [ -f /root/.zshrc ] && ! sudo grep -q '\[ -f ~/.zsh_aliases \] && source ~/.zsh_aliases' /root/.zshrc; then
