...
if [[ -z "$1" ]]; then
    ...
    if [[ "$fzf_key" == 'y' ]]; then
        ...
        [ -f $ZSH_KEYBIND_FILEDIR/fzf-bindings.zsh ] && 
            rm $ZSH_KEYBIND_FILEDIR/fzf-bindings.zsh
        wget-aria-name ~/.keybinds.d/fzf-bindings.zsh https://raw.githubusercontent.com/junegunn/fzf/refs/heads/master/shell/key-bindings.zsh
    fi
