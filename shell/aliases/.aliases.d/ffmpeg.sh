...
if hash ffmpeg &> /dev/null; then
    ...
    function ffmpeg-convert-to-mp3(){
        ...
        for var in "$@"
        do
            ffmpeg -i "$var" -vn -acodec libmp3lame -ac 2 -ab 160k -ar 48000 "${var%.*}.mp3" && test "$del" == 'y' && test -f "$var" && command rm -v "$var" 
        done
    }
    ...
    function ffmpeg-convert-mkv-embed-subtitles(){
        ...
        do
            ...
            test "$del" == 'y' && test -f $sub && command rm -v $sub  
        done
    }
    ...
    function ffmpeg-convert-to-mp4(){
        ...
        do
            ...
            ffmpeg -i "$var" $sub "${var%.*}.mp4" && test "$del" == 'y' && test -f "$var" && command rm -v "$var" 
            test "$del" == 'y' && test -f $sub && command rm -v $sub  
        done
        unset del sub var 
    }
