...
hash fd &> /dev/null && SYSTEM_UPDATED='TRUE'
...
if hash fd-find &> /dev/null || hash fd &> /dev/null; then
    ...
    if [[ $fndgbl == 'y' ]]; then
        if ! test -f install_gitignore.sh; then
            b=$(mktemp)
            wget-curl https://raw.githubusercontent.com/excited-bore/dotfiles/main/cli-tools/install_gitignore.sh | tee "$b" &> /dev/null
            chmod u+x "$b"
            eval "$b" "global"
            unset b
        else
